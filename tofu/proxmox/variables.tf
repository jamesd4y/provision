# Everything except the credentials comes from tofu/proxmox/vars/<node>.tfvars.json,
# which scripts/render_tfvars.py generates from hosts/<node>/.

variable "proxmox_api_token" {
  description = "Proxmox API token (user@realm!tokenid=uuid), injected from Bitwarden Secrets Manager by CI."
  type        = string
  sensitive   = true
}

variable "proxmox_ssh_username" {
  description = "SSH user on the Proxmox node. The provider needs it to import disk images and upload snippets."
  type        = string
  default     = "root"
}

variable "proxmox_ssh_password" {
  description = "SSH password for that user, injected from Bitwarden Secrets Manager by CI."
  type        = string
  sensitive   = true
  default     = ""
}
...
variable "endpoint" {
  description = "Proxmox API endpoint: { url, node, insecure }."
  type        = any
}
